...
export default class CustomSideBarMenu extends Component{
  state = {
   ...
   return ref.put(blob).then((response)=>{this.fecthImage(imageName)})
   }

fecthImage=(imageName)=>{
  var ref = firebase.storage().ref().child('user_profiles/'+imageName)
  ref.getDownloadURL().then((url)=>{this.setState({
    image:url
  })})
  .catch((error)=>{
    this.setState({
...
componentDidMount(){
  this.fecthImage(this.state.userId)
  this.getUserProfile()
